...
ipcMain.handle('export-absence-excel', async (event, { date, data }) => {
  ...
  try {
    ...
    function commLabel(r) {
      if (r.drp_Communicate && r.drp_Communicate.trim()) return 'à¹‚à¸—ร';
      if (r.drp_Communicate1 && r.drp_Communicate1.trim()) return 'à¹à¸ˆà¹‰à¸‡à¸¥à¹ˆà¸§à¸‡à¸«à¸™à¹‰า';
      return '';
    }
    ...
    function fillPage(sheet, pageIdx) {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (i < velPage.length) {
          ...
          sheet.getCell(rowNum, 7).value  = comm === 'à¹‚à¸—ร' ? '✓' : '';
          sheet.getCell(rowNum, 8).value  = comm === 'à¹à¸ˆà¹‰à¸‡à¸¥à¹ˆà¸§à¸‡à¸«à¸™à¹‰า' ? '✓' : '';
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (i < outPage.length) {
          ...
          sheet.getCell(rowNum, 19).value = comm === 'à¹‚à¸—ร' ? '✓' : '';
          sheet.getCell(rowNum, 20).value = comm === 'à¹à¸ˆà¹‰à¸‡à¸¥à¹ˆà¸§à¸‡à¸«à¸™à¹‰า' ? '✓' : '';
